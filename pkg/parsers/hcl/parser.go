...
type DinghyfileHclParser struct {
	Builder *dinghyfile.PipelineBuilder
}

func NewDinghyfileHclParser(b *dinghyfile.PipelineBuilder) *DinghyfileHclParser {
	return &DinghyfileHclParser{Builder: b}
}

func (r *DinghyfileHclParser) SetBuilder(b *dinghyfile.PipelineBuilder) {
	r.Builder = b
}

func (r *DinghyfileHclParser) varFunc(vars []dinghyfile.VarMap) interface{} {
...
func (r *DinghyfileHclParser) pipelineIDFunc(vars []dinghyfile.VarMap) interface{} {
				...
				if deepVariable[0:5] == "{{var" {
					for _, vm := range allVars {
						if val, exists := vm[deepVariable[6:len(deepVariable)-2]]; exists {
							r.Builder.Logger.Info("Substituting deepvariable ", vars[i], " : old value : ", deepVariable, " for new value: ", val.(string))
							vars[i+1] = val
						}
					}
				}
...
func (r *DinghyfileHclParser) makeSlice(args ...interface{}) []interface{} {
	return args
}
